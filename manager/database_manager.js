...
async function get_discussion(stuff_id, limit, cnt) {
    ...
    if (stuff_id == null || stuff_id == '') {
        let query = "SELECT tbl_discussion.*, COUNT(tbl_comment.id) as comment_cnt from tbl_discussion LEFT JOIN tbl_comment ON tbl_discussion.id = tbl_comment.discussion_id GROUP BY tbl_discussion.id ORDER BY tbl_discussion.likes DESC LIMIT ?, ?";
        [rows, ] = await mysql_execute(connection, query, [limit, cnt]);
    } else {
        let query = "SELECT tbl_discussion.*, COUNT(tbl_comment.id) as comment_cnt  from tbl_discussion LEFT JOIN tbl_comment ON tbl_discussion.id = tbl_comment.discussion_id WHERE stuff_id LIKE ? GROUP BY tbl_discussion.id ORDER BY tbl_discussion.likes DESC LIMIT ?, ?";
...
async function get_discussion_by_keyword(stuff_id, limit, cnt, keyword) {
    let connection = await connect();
    let query = "SELECT * from tbl_discussion WHERE tbl_discussion.content LIKE ? AND tbl_discussion.stuff_id LIKE ? ORDER BY tbl_discussion.likes DESC LIMIT ?, ?";
    let [rows, ] = await mysql_execute(connection, query, ['%' + keyword + '%', stuff_id, limit, cnt]);
...
async function get_discussion_by_id(id) {
    ...
    let query = "SELECT tbl_discussion.*, COUNT(tbl_comment.id) as comment_cnt from tbl_discussion LEFT JOIN tbl_comment ON tbl_discussion.id = tbl_comment.discussion_id WHERE tbl_discussion.id LIKE ? GROUP BY tbl_discussion.id";
...
async function add_discussion(stuff_id, content, user_type, user) {
    ...
    try {
        ...
        let query = "INSERT INTO tbl_discussion (stuff_id, content, user, user_type) VALUE (?,?,?,?)";
        let [rows, ] = await mysql_execute(connection, query, [stuff_id, content, user, user_type]);
...
async function add_comment(discussion_id, parent_id, content, user_type, user) {
    ...
    try {
        ...
        let query = "INSERT INTO tbl_comment (discussion_id, parent_id, content, user, user_type) VALUE (?,?,?,?,?)";
        let [rows, ] = await mysql_execute(connection, query, [discussion_id, parent_id, content, user, user_type]);
...
async function add_token(connection, item) {
    ...
    try {
        let query = "INSERT INTO tbl_item (game_id, category_id, contract_address, token_id, name, description, attach_url, owner, is_anonymous, arcadedoge_price) VALUE(?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
        let [rows, ] = await mysql_execute(connection, query, [item.game_id, item.category_id, item.contract_address, item.token_id, item.name, item.description, item.attach_url, item.owner, item.is_anonymous, item.arcadedoge_price]);
        ret = rows.insertId;
    } catch (err) {
        console.log(err);
    }
...
async function add_mint_tx(connection, id, item) {
    ...
    try {
        let query = "INSERT INTO tbl_history (token_id, from_address, to_address, type) VALUE(?, ?, ?, ?)";
        let [rows, ] = await mysql_execute(connection, query, [id, item.owner, item.owner, CONST.TX_TYPE.MINT]);
        ret = rows.insertId > 0;
    } catch (err) {
        console.log(err);
    }
...
async function update_sync_block_number(connection, contract_type, block_number) {
    ...
    try {
        let query = "UPDATE tbl_status SET block_number = ? WHERE contract_type = ?";
        let [rows, ] = await mysql_execute(connection, query, [block_number, contract_type]);
...
async function get_sync_block_number(contract_type) {
    ...
    try {
        let query = "SELECT block_number FROM tbl_status WHERE contract_type = ?";
...
async function update_token_by_id(id, game_id, category_id, name, description, is_anonymous, price) {
    ...
    try {
        let query = "UPDATE tbl_item SET game_id=?, category_id=?, name=?, description=?, is_anonymous=?, arcadedoge_price=? WHERE tbl_item.id LIKE ?";
        let [rows, ] = await mysql_execute(connection, query, [game_id, category_id, name, description, is_anonymous, price, id]);
        ret = (rows.affectedRows > 0)
    } catch (err) {
        console.log(err);
    }
...
async function get_token_by_contract_info(contract_address, token_id) {
    ...
    try {
        let query = "SELECT * from tbl_item WHERE contract_address = ? AND token_id = ?";
        let [rows, ] = await mysql_execute(connection, query, [contract_address, token_id]);
        if (rows.length > 0) ret = rows[0];
    } catch (err) {
        console.log(err);
    }
...
async function delete_token(connection, id) {
    ...
    try {
        ...
        let [rows, ] = await mysql_execute(connection, query, [CONST.BURN_STATUS.BURNT, id]);
        ret = rows.affectedRows > 0;
    } catch (err) {
        console.log(err);
    }
...
async function add_burn_tx(connection, item) {
    ...
    try {
        let query = "INSERT INTO tbl_history (token_id, from_address, to_address, type) VALUE (?, ?, ?, ?)";
        let [rows, ] = await mysql_execute(connection, query, [item.id, item.owner, item.owner, CONST.TX_TYPE.BUNRT]);
        ret = rows.insertId > 0;
    } catch (err) {
        console.log(err);
    }
...
async function bunr_token(contract_address, token_id, block_number) {
    ...
    try {
        ...
        let token = await get_token_by_contract_info(contract_address, token_id);
...
async function update_token_visible(connection, id, visible, arcadedoge_price = 0) {
    ...
    try {
        if (visible == CONST.VISIBILITY_STATUS.SHOW) {
            let query = "UPDATE tbl_item SET is_visible = ?, arcadedoge_price = ? WHERE id = ?";
            let [rows, ] = await mysql_execute(connection, query, [visible, arcadedoge_price, id]);
            ret = rows.affectedRows > 0;
...
async function update_token_owner(connection, id, owner) {
    ...
    try {
        let query = "UPDATE tbl_item SET owner = ?, is_visible = false WHERE id = ?";
...
async function add_exchange_tx(connection, id, from, to, asset_id, amount) {
    ...
    try {
        let query = "INSERT INTO tbl_history (token_id, from_address, to_address, asset_id, amount, type) VALUE(?, ?, ?, ?, ?, ?)";
        let [rows, ] = await mysql_execute(connection, query, [id, from, to, asset_id, amount, CONST.TX_TYPE.EXCHANGE]);
        ret = rows.insertId > 0;
    } catch (err) {
        console.log(err);
    }
...
async function add_transfer_tx(connection, id, from, to) {
    ...
    try {
        let query = "INSERT INTO tbl_history (token_id, from_address, to_address, type) VALUE(?, ?, ?, ?)";
        let [rows, ] = await mysql_execute(connection, query, [id, from, to, CONST.TX_TYPE.EXCHANGE]);
        ret = rows.insertId > 0;
    } catch (err) {
        console.log(err);
    }
...
async function increase_trade_cnt(connection, id) {
    ...
    try {
        let query = "UPDATE tbl_item SET trade_cnt = trade_cnt + 1 WHERE id = ?";
        let [rows, ] = await mysql_execute(connection, query, [id]);
        ret = rows.affectedRows > 0;
    } catch (err) {
        console.log(err);
...
async function get_items_by_address(address, sort_type, limit, cnt) {
    let connection = await connect();
    let query = "SELECT tbl_item.*, tbl_category.name as category_name from tbl_item LEFT JOIN tbl_category ON tbl_item.category_id = tbl_category.id WHERE is_burnt = 0 AND owner = ? " + get_order_by_clause(sort_type) + " LIMIT ?, ?";
...
async function get_items_by_address_cnt(address) {
    let connection = await connect();
    let query = "SELECT COUNT(id) as total from tbl_item WHERE is_burnt = 0 AND owner = ?";
...
async function get_market_items(game, category, sort_type, limit, cnt) {
    ...
    let query = "SELECT tbl_item.*, tbl_category.name as category_name from tbl_item LEFT JOIN tbl_category ON tbl_item.category_id = tbl_category.id WHERE is_visible = ? AND is_burnt = 0";
...
async function get_market_items_cnt(game, category) {
    ...
    let query = "SELECT COUNT(id) as total from tbl_item WHERE is_visible = ? AND is_burnt = 0";
    let params = [CONST.VISIBILITY_STATUS.SHOW];
...
async function insert_likes(discussion_id, parent_id, user) {
    ...
    try {
        let query = "INSERT INTO tbl_likes (discussion_id, parent_id, user) VALUE(?, ?, ?)";
        let [rows, ] = await mysql_execute(connection, query, [discussion_id, parent_id, user]);
        ret = rows.insertId > 0;
    } catch (err) {
        console.log(err);
    }
...
async function delete_likes(discussion_id, parent_id, user) {
    ...
    try {
        let query = "DELETE FROM tbl_likes WHERE discussion_id LIKE ? AND parent_id LIKE ? AND user LIKE ?";
        let [rows, ] = await mysql_execute(connection, query, [discussion_id, parent_id, user]);
        ret = rows.affectedRows > 0;
    } catch (err) {
        console.log(err);
...
async function get_likes(discussion_id, parent_id, user) {
    let connection = await connect();
    let query = "SELECT id from tbl_likes WHERE discussion_id LIKE ? AND parent_id LIKE ? AND user LIKE ?";
    let [rows, ] = await mysql_execute(connection, query, [discussion_id, parent_id, user]);
    connection.end();
    return rows;
}
...
async function get_likes_count(discussion_id, parent_id) {
    let connection = await connect();
    let query = "SELECT COUNT(id) as total from tbl_likes WHERE discussion_id LIKE ? AND parent_id LIKE ?";
    let [rows, ] = await mysql_execute(connection, query, [discussion_id, parent_id]);
    connection.end();
    return rows[0].total;
}
