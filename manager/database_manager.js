...
async function get_discussion(stuff_id, limit, cnt) {
    ...
    if (stuff_id == null || stuff_id == '') {
        let query = "SELECT * from tbl_discussion ORDER BY tbl_discussion.likes DESC LIMIT ?, ?";
        [rows, ] = await mysql_execute(connection, query, [limit, cnt]);
    } else {
        let query = "SELECT * from tbl_discussion WHERE stuff_id LIKE ? ORDER BY tbl_discussion.likes DESC LIMIT ?, ?";
