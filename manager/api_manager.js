...
function register_apis(app) {
    app.post ("/stuff/all", async(req, res) => {
        ...
        for(let i = 0; i < stuffs.length; i ++) {
            ...
            for (let j = 0; j < discussions.length; j ++) {
                let likes_count = await database_manager.get_likes_count(discussions[j].id, -1)
    ...
    app.post("/discussion/all/", async(req, res) => {
        ...
        for (let i = 0; i < discussions.length ; i ++) {
            const likes_count = await database_manager.get_likes_count(discussions[i].id, -1)
    ...
    app.post("/discussion", async(req, res) => {
        ...
        const likes_count = await database_manager.get_likes_count(discussion.id, -1)
        discussion.likes = likes_count
        if (discussion != null) {
            let comments = await database_manager.get_comment(id);
            
            for (let i = comments.length - 1; i >= 0; i --) {
                const likes_count = await database_manager.get_likes_count(comments[i].discussion_id, comments[i].id)
    ...
    app.post("/set_likes", async(req, res) => {
        ...
        if (likesOrUnlikes == true) {
            result = await database_manager.insert_likes(discussion_id, parent_id, user);
        } else {
            result = await database_manager.delete_likes(discussion_id, parent_id, user);
        }
    ...
    app.post("/get_likes", async(req, res) => {
        ...
        let result = await database_manager.get_likes(discussion_id, parent_id, user);
